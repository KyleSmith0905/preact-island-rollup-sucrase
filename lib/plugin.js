...
function generateIslandsWithSource(sourceCode, filepath, options) {
  ...
  if (!(options.server && options.server.output)) {
    options.server = options.server || {}
    options.server.output = resolve(workingDir, 'server')
  }

  let clientOutput = resolve(options.client.output, path.basename(filepath))
  let serverOutput = resolve(options.server.output, path.basename(filepath))
  const fileExt = extname(filepath)

  if (options.hash) {
    const hash = toHash(sourceCode)
    clientOutput = clientOutput.replace(fileExt, `-${hash}.js`)
    serverOutput = serverOutput.replace(fileExt, `-${hash}.js`)
  } else {
    clientOutput = clientOutput.replace(fileExt, `.js`)
    serverOutput = serverOutput.replace(fileExt, `.js`)
  }
  ...
  const clientCode = constructIslandClient(componentName, {
  ...
  result.paths.server = serverOutput
...
function constructIslandClient(
  name,
  { viewMods, replaceParentNode = false, server } = {}
) {
  const islandName = getIslandName(name)
  const scriptName = server.output
  const scriptBaseName = path.basename(scriptName)

  const importPath = `../server/${scriptBaseName}`
...
function constructIslandServer(ast, name, options = {}) {
  ...
  const scriptClientPath = path
    .join(
      'public/__preact_islands/',
      client.output.split('__preact_islands')[1]
    )
    .replace(/\\/g, '/')
  /* 
  ...
  if (options.atomic) {
    ...
    export default function Island${name}(props) {
      return h(Fragment,{},
        ...
        h("script",{async:true, src:"${scriptClientPath}", type:"module"}),
      )
    }
  `
  } else {
